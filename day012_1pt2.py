...
def dijkstra_algorithm(grid, position):
    unvisited_nodes = [(row, col) for row in range(len(grid)) for col in range(len(grid[0]))]
...
previous_nodes, shortest_path = dijkstra_algorithm(grid, start_position)
end_position = find_end(grid)
print_result(previous_nodes, shortest_path, start_position, end_position)
